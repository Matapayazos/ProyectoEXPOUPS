...
import ec.edu.ups.bussiness.NoticiaBussiness;
...
public class NoticiaBean {
	...
	@Inject
	private NoticiaBussiness carBussines;
	...
	public void init() {
		...
		listaNoticias = carBussines.getNoticias();
		edit = false;
		id = 0;
	...
	public void loadData() {
		...
		if (id == 0)
			return;
		try {
			newNoticia = carBussines.getNoticia(id);//
			edit = true;
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String guardar() {
		try {
			...
			if (edit) {
				carBussines.update(newNoticia);
				System.out.println("Registro Modificado");
			} else {
				Carrera auxCarrera = cliBussiness.getCarrera(idCarrera);
				newNoticia.setCarrera(auxCarrera);

				carBussines.save(newNoticia);
				System.out.println("Registro Guardado");
			}
			return "Create-Noticia?faces-redirect=true";
		} catch (Exception e) {
			e.printStackTrace();
			FacesMessage m = new FacesMessage(FacesMessage.SEVERITY_ERROR, e.getMessage(), "Error");
			facesContext.addMessage(null, m);
	...
	public String editar(Noticia car) {
		edit = true;
		return "Create-Noticia?faces-redirect=true&id=" + car.getId();
	}
...
//
	...
	public NoticiaBussiness getCarBussines() {
		return carBussines;
	}

	public void setCarBussines(NoticiaBussiness carBussines) {
